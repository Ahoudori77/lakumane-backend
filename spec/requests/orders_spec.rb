...
RSpec.describe "Orders API", type: :request do
  ...
  describe "POST /orders" do
    context "正常系" do
      it "新しい発注を作成できる" do
        post "/orders", params: { order: { item_id: item.id, quantity: 10, status: "pending" } }, headers: auth_headers
    ...
    context "異常系" do
      it "quantityが0以下の場合エラーを返す" do
        post "/orders", params: { order: { item_id: item.id, quantity: 0, status: "pending" } }, headers: auth_headers
        expect(response).to have_http_status(:unprocessable_entity)
        expect(JSON.parse(response.body)["errors"]).to include("Quantity must be greater than 0")
      end

      it "statusが不正な値の場合エラーを返す" do
        post "/orders", params: { order: { item_id: item.id, quantity: 10, status: "invalid_status" } }, headers: auth_headers
        expect(response).to have_http_status(:unprocessable_entity)
