...
RSpec.describe "Inventory API", type: :request do
  ...
  describe "GET /inventory" do
    it "在庫リストを取得できる" do
      get "/inventory", headers: auth_headers
  ...
  describe "PATCH /inventory/:id" do
    context "正常系" do
      it "在庫数を増やすことができる" do
        patch "/inventory/#{item.id}", params: { quantity: 5 }, headers: auth_headers
        expect(response).to have_http_status(:ok)
    ...
    context "異常系" do
      it "在庫数が負の値になる場合エラーを返す" do
        patch "/inventory/#{item.id}", params: { quantity: -20 }, headers: auth_headers
        expect(response).to have_http_status(:unprocessable_entity)
        expect(JSON.parse(response.body)["error"]).to eq("Quantity cannot be negative")
      end

      it "存在しないアイテムの場合エラーを返す" do
        patch "/inventory/0", params: { quantity: 5 }, headers: auth_headers
