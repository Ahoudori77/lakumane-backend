...
RSpec.describe "Items API", type: :request do
  ...
  describe "POST /items" do
    context "有効なパラメータの場合" do
      it "アイテムを作成できる" do
        ...
        post '/items', params: valid_params
    ...
    context "無効なパラメータの場合" do
      it "エラーを返す" do
        ...
        post '/items', params: invalid_params
    ...
    context "有効なパラメータの場合" do
     it "アイテムを更新できる" do
       valid_params = {
        ...
        patch "/items/#{item.id}", params: valid_params
        expect(response).to have_http_status(:ok)
  ...
  context "無効なパラメータの場合" do
    it "エラーを返す" do
      ...
      patch "/items/#{item.id}", params: invalid_params
      expect(response).to have_http_status(:unprocessable_entity)
      expect(JSON.parse(response.body)).to include("errors")
    end
  end
  ...
  context "存在しないアイテムの場合" do
    it "エラーを返す" do
      patch "/items/0", params: { item: { name: "存在しないアイテム" } }
      expect(response).to have_http_status(:not_found)
      expect(JSON.parse(response.body)).to include("error" => "Item not found")
    end
  end
